    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # --------------------------------------------------------------
                # score = ...
                arr_goal = ([[goal_state[0], goal_state[1]]])
                arr_path_end = ([[paths[i][0][-1], paths[i][1][-1]]])
                score = abs(scipy.spatial.distance.cdist(arr_goal, arr_path_end))
                # print(arr_goal)
                # print(arr_path_end)
                # print(score)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            # --------------------------------------------------
                            # score += self._weight * ...
                            arr_path1_end = arr_path_end
                            arr_path2_end = ([[paths[i][0][-1], paths[i][1][-1]]])
                            score += -abs(self._weight * scipy.spatial.distance.cdist(arr_path1_end, arr_path2_end))
                            # --------------------------------------------------
