...
class BehaviouralPlanner:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        #   arc_length += ...
        while wp_index < len(waypoints) - 1:
            arc_length += ( (waypoints[wp_index+1][0]-waypoints[wp_index][0])**2\
                + (waypoints[wp_index+1][1]-waypoints[wp_index][1])**2)**0.5
            wp_index += 1
            if arc_length > self._lookahead :
                return wp_index
        # ------------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    #   ...
    for i in range(len(waypoints)):
        dis = ((ego_state[0]-waypoints[i][0])**2+(ego_state[1]-waypoints[i][1])**2)**0.5
        angle = np.arctan2((waypoints[i][1]-ego_state[1]), (waypoints[i][0])-ego_state[0])-ego_state[2]
        if angle>2*math.pi:
            angle = 2*math.pi-angle
        elif angle < -2*math.pi:
            angle = angle+2*math.pi
        print(angle*180/math.pi)
        if dis < closest_len and (abs(angle)<math.pi/2 or  abs(angle)>3*math.pi/2):
            closest_len = dis
            closest_index = i
    # ------------------------------------------------------------------
