...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # ------------------------------------------------------------------
        # bounds = ...
        bound = ((-0.5, 0.5), (-0.5, 0.5), (sf_0, None))
        ...
        res = scipy.optimize.minimize(fun=self.objective, x0=p0, method ='L-BFGS-B',\
             jac=self.objective_grad, bounds=bound, options={'disp': True})
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        x_points = 0 + scipy.integrate.cumtrapz(np.cos(t_points), s_points, initial=0)
        y_points = 0 + scipy.integrate.cumtrapz(np.sin(t_points), s_points, initial=0)
        return [x_points, y_points, t_points]
        # ------------------------------------------------------------------
