...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        # goal_y = ...
        theta = -ego_state[2]
        rotate_mat = np.array([[cos(theta), -sin(theta)],[sin(theta), cos(theta)]])
        # rotate_mat = np.array([[np.cos(theta), -np.sin(theta)],[np.sin(theta), np.cos(theta)]])
        goal_xy = rotate_mat @ ([goal_state_local[0]],[goal_state_local[1]])
        # goal_xy = rotate_mat @ np.array([[goal_state_local[0]],[goal_state_local[1]]])
        goal_x = goal_xy[0][0]
        goal_y = goal_xy[1][0]
        # print(goal_x, goal_y)
        # ------------------------------------------------------------------
