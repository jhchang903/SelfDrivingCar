...
TOTAL_RUN_TIME         = 110.00 # game seconds (total runtime before sim end)
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        lp_st = lv.LivePlotter(tk_title="Steering")
        lp_thr = lv.LivePlotter(tk_title="Throttle")
        ...
        ### My plot
        steer_fig2 = lp_st.plot_new_dynamic_2d_figure(
                title='Steering Angle (rad)',
                figsize=(FIGSIZE_X_INCHES-3, FIGSIZE_Y_INCHES-3),
                edgecolor="black",
                rect=[PLOT_LEFT, PLOT_BOT, PLOT_WIDTH, PLOT_HEIGHT])
        steer_fig2.add_graph("steer", 
                              label="steer", 
                              color='b',
                              window_size=TOTAL_EPISODE_FRAMES)


        throttle_fig2 = lp_thr.plot_new_dynamic_2d_figure(
                title='Throttle (%)',
                figsize=(FIGSIZE_X_INCHES-3, FIGSIZE_Y_INCHES-3),
                edgecolor="black",
                rect=[PLOT_LEFT, PLOT_BOT, PLOT_WIDTH, PLOT_HEIGHT])
        throttle_fig2.add_graph("throttle", 
                              label="throttle", 
                              color='b',
                              window_size=TOTAL_EPISODE_FRAMES)
        ...
        if not enable_live_plot:
            ...
            lp_st._root.withdraw() 
            lp_thr._root.withdraw()        
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            if skip_first_frame and frame == 0:
                ...
                throttle_fig2.roll("throttle", current_timestamp, cmd_throttle)
                steer_fig2.roll("steer", current_timestamp, cmd_steer*1.22)
        ...
        store_trajectory_plot(throttle_fig2.fig, 'throttle_output2.png')
        store_trajectory_plot(brake_fig.fig, 'brake_output.png')
        store_trajectory_plot(steer_fig.fig, 'steer_output.png')
        store_trajectory_plot(steer_fig2.fig, 'steer_output2.png')
